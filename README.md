...
office a reply names is competent to handle the matter — are written up in the volunteered
limitations on `/how-this-works`, which are numbered at render time; naming them by number here
was already wrong once. So is a third: the same reply does not always get the same verdict, and
every percentage above comes from a single run.
